...
public class Arcade extends Unit {
    ...
    public void process() {
        if (!paused) {
            ...
            if (current.landed) {
                ...
                cubbies[ccolumn].lastSwiTime = System.currentTimeMillis();
            ...
            if (y >= 0) {
                ...
                if (x != ccolumn) {
                    ...
                    if (holding.tile == null) {
                        ...
                        holding.yoffset = touchPos.y - holding.ypos - size / 2;
                        holding.held = true;
                    } else {
            ...
            if (tile.ypos != tile.coords.y * tileSize) {
                if (tile.ypos > tile.coords.y * tileSize) {
                    if (tile.ypos - tile.velocity > tile.coords.y * tileSize) {
                        ...
                        tile.velocity = tile.velocity + 1f;
                    } else {
                        tile.landed = true;
    ...
    public void updateCubbies() {
        ...
        for (int i = 0; i < field.length; i++) {
            ...
            if (!cubby.held) {
                ...
                } else if (cubby.tile == null) {
                    if (holding != null && cubby != holding) {
                        time = (System.currentTimeMillis() - holding.lastChaTime) / 250f;
                        cubby.ypos = -tileSize;
                        if (time <= 1) {
                            cubby.ypos = new Interpolation.SwingIn(1.5f).apply(cubby.oPos.y, -tileSize, time);
                        }
                    } else if (lastHeld != null && cubby != lastHeld) {
                        time = (System.currentTimeMillis() - lastHeld.lastMovTime) / 250f;
                        cubby.ypos = cubby.oPos.y;
                        if (time <= 1) {
                            cubby.ypos = new Interpolation.SwingOut(1.5f).apply(-tileSize, cubby.oPos.y, time);
                        }
                    }
                }
            } else {
                ...
                if (time <= 1) {
                    ...
                    cubby.xpos = new Interpolation.SwingOut(1.5f).apply(cubby.oPos.x, cubby.tPos.x, time);
                    cubby.ypos = new Interpolation.SwingOut(1.5f).apply(cubby.oPos.y, cubby.tPos.y, time);
                }
            }
        }
    }
        ...
        for (int i = 0; i < field.length; i++) {
            ...
            if (cubby != holding && cubby.tile != null) {
                ...
                if (i != ccolumn) {
                    ...
                    float time = (System.currentTimeMillis() - cubby.lastSwiTime) / 250f;
                    float scale = new Interpolation.SwingOut(1.5f).apply(1, 0.2f, time);
                    if (time <= 1) {
                        game.batch.draw(new TextureRegion(game.pixel), cubby.oPos.x - ((size - cubby.size)/2), cubby.oPos.y -  ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), cubby.size, cubby.size, scale, scale, 0);
                        game.batch.setColor(Color.WHITE);
                        scale = new Interpolation.SwingOut(1.5f).apply(1, 0, time);
                        game.batch.draw(new TextureRegion(game.types, game.types.getHeight() * cubby.tile.type, 0, game.types.getHeight(), game.types.getHeight()), cubby.oPos.x, cubby.oPos.y, tileSize / 2, tileSize / 2, cubby.size, cubby.size, scale, scale, cubby.tile.angle);
                    } else {
                        game.batch.draw(new TextureRegion(game.pixel), cubby.oPos.x -  ((size - cubby.size)/2), cubby.oPos.y -  ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), size, size, 0.2f, 0.2f, 0);
                        game.batch.setColor(Color.WHITE);
                        game.batch.draw(new TextureRegion(game.types, game.types.getHeight() * cubby.tile.type, 0, game.types.getHeight(), game.types.getHeight()), cubby.oPos.x, cubby.oPos.y, tileSize / 2, tileSize / 2, size, size, 0, 0, cubby.tile.angle);
                    }
                }
            }
            ...
            game.batch.draw(new TextureRegion(game.pixel), cubby.xpos + ((size - cubby.size)/2), cubby.ypos + ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), 5, cubby.size, 1, 1, 0);
            game.batch.draw(new TextureRegion(game.pixel), cubby.xpos + ((size - cubby.size)/2), cubby.ypos + cubby.size - 5 + ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), cubby.size, 5, 1, 1, 0);
            game.batch.draw(new TextureRegion(game.pixel), cubby.xpos + cubby.size - 5 + ((size - cubby.size)/2), cubby.ypos + ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), 5, cubby.size, 1, 1, 0);
            game.batch.draw(new TextureRegion(game.pixel), cubby.xpos + ((size - cubby.size)/2), cubby.ypos + ((size - cubby.size)/2), (tileSize / 2), (tileSize / 2), cubby.size, 5, 1, 1, 0);
    ...
    private void drawBlack() {
        if (holding != null) {
            ...
            game.batch.draw(new TextureRegion(game.pixel), holding.xpos + ((size - holding.size) / 2) - 10, holding.ypos + ((size - holding.size) / 2) - 10, (tileSize / 2), (tileSize / 2), holding.size + 20, holding.size + 20, 1, 1, 0);
        }
    }
    ...
    private void drawWalls() {
        if (dframes > 0) {
            ...
            game.batch.setColor(Color.RED);
            game.batch.draw(new TextureRegion(game.pixel), 0, yoffset + tileSize * (ceiling - 1), (tileSize / 2), (tileSize / 2), game.camera.viewportHeight - tileSize*2, 5, 1, 1, 0);
        }
        game.batch.setColor(Color.WHITE);
        ...
        game.header.draw(game.batch, Integer.toString(score), xoffset, game.camera.viewportHeight - tileSize);
    }
